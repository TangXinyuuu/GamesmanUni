...
export const generateComputerMove = (app: Types.App) => {
    const useWinby = app.currentMatch.useWinby
    const showWinby = app.options.showWinby
    const round = app.currentMatch.round
    ...
    if (showWinby) {
        if (currentPositionValue === "win" || currentPositionValue === "tie") {
            ...
            bestMoves = availableMoves.filter((availableMove) => availableMove.winby === maximumWinby);
        }
