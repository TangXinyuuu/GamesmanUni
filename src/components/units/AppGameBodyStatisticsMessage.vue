    ...
    <div id="app-game-body-statistics-message" v-if="showNextMoveHints">
        <template v-if="currentRemoteness != 0">
            ...
            <p v-else-if="!isPuzzleGame">
                ...
                the game in {{ currentRemoteness }} move<span v-if="currentRemoteness !== 1">s</span>
                {{ winbyStr }}. {{ mexStr }}
            </p>
            <p v-else>
                <b :class="`uni-turn-${currentTurn}`">{{ currentPlayerName }}</b> should 
...
<script lang="ts" setup>
    ...
    const winbyStr = computed(() => store.getters.currentWinby === undefined ? "" : " by " + Math.abs(store.getters.currentWinby));
